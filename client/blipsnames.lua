...
CreateThread(function()
    while true do
        ...
        if NetCheck1 or NetCheck2 then
            TriggerServerEvent('qz-admin:server:GetPlayersForBlips')
        end
    end
end)

RegisterNetEvent('qz-admin:client:toggleBlips', function()
    if not ShowBlips then
        ShowBlips = true
        NetCheck1 = true
        ESX.ShowNotification("Los blips de los jugadores han sido habilitados", "success")
    else
        ShowBlips = false
        ESX.ShowNotification("Blips habilitados", "error")
    end
end)

RegisterNetEvent('qz-admin:client:toggleNames', function()
    if not ShowNames then
        ShowNames = true
        NetCheck2 = true
        ESX.ShowNotification("Nombres activados", "success")
    else
        ShowNames = false
        ESX.ShowNotification("Nombres desactivados", "error")
    end
end)

RegisterNetEvent('qz-admin:client:Show', function(players)
